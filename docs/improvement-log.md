...
- [content] Consistency review Tier 2 (style/terminology): convert `diagnostic-yields` plain-text citations (Zhang 2024, Wright 2023, Gonzalez-Mantilla 2023, Srivastava 2022, Pandey 2025, Nurchis 2023, Fogel 2020, Zerem 2023, Manickam) to linked `[Author Year](pubmed)`; migrate remaining "dysmorphic/dysmorphism" → "distinctive features" (variant-interpretation, neuroimaging, diagnostic-yields, gdd-id-asd); normalize ES/GS → WES/WGS in `intro-neurogenetics`; lowercase "Syndrome"/"Complex" in `neurodevelopmental-disorders`; pick "&" vs "and" in module titles; temper bare superlatives (cnv-interpretation "highest yield for any single variant class", pharmacogenetics "most critical drug contraindication in pediatric neurology", intro "highest-yield 'test'"); standardize Williams vs Williams-Beuren
- [housekeeping] Consistency review Tier 3: redistribute the "always option B" answer key in 5 quizzes (`central-dogma` final, `chromosomes-iscn` final, `iem` final, `mitochondrial` inline, `therapies` inline); optional — add explicit `slideDistribution` to the 3 modules whose deck doesn't divide evenly across sections (`chromosomes-iscn` 9/5, `clinical-reasoning` 14/6, `genetic-counseling` 11/6)
## History
